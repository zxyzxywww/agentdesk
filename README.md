...
状态库放在工作区外（与生产同构）。前 8 类覆盖确定性文件/表格/检索/多步能力面，后 2 类为多约束复杂任务
（审计报告四小节齐全且问题≥3；验收清单 5 条逐条判定）。
...
|---|---|---|
| 执行次数 | 24 | 24 |
| 任务完成率 / 产物达标率 | 100% / 100% | 100% / 100% |
| 首轮直接通过率（无返工即达标） | 100% | 100% |
| 平均返工次数 | — | 0.00 |
| 平均步数 / 平均成本（元） | 5.0 / 0.0323 | 5.0 / 0.0321 |
| 平均耗时 | 8s | 8s |
| 人工确认次数（HITL 拦截） | 6 | 5 |
...
> 暴露并修复了一个真实缺陷——多工具调用中一个触发确认时其余调用缺 tool 响应会让 DeepSeek 报 400
> （补占位响应解决，回归测试覆盖）。
> 复现：`uv run python scripts/bench_reflection.py --tasks 10 --repeat 3 --out <文件>`（60 次，约 ¥3，需账户余额）。
